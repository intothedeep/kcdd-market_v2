...
- **Data access / RLS portability posture**:
  - **Freeze new direct browser queries.** New data access from the frontend goes through `frontend-vite/src/lib/api.ts` (authenticated Express API), NOT new `supabase.from(...)` calls in the browser. The ~138 existing direct queries stay for now (RLS-enforced), but do not add more — this caps the future Option-C migration cost. (Ref: `docs/rls-authz-portability.md`.)
  - **New sensitive features prefer the API path.** Anything touching money, PII, or admin actions must be served by the backend API (which verifies the Clerk JWT and uses the service-role key), not by direct browser Supabase access. Payments already follow this.
  - **Never expose the service-role key to the browser**, and treat `public.clerk_user_id()`'s dependency on the `request.jwt.claims` GUC (injected by Supabase Third-Party Auth) as THE single Supabase-portability chokepoint — keep it documented.
